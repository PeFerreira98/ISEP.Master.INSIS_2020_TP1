...
function generateForm(data, disabled) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        } else if (data[i].type == "simpleSelect") {
            formContent += genSimpleSelect(data[i], disabled);
        } else if (data[i].type == "simpleTable") {
            formContent += genSimpleTable(data[i], disabled);
        }
    }
    return formContent;
}

function genSimpleSelect(data,  disabled)   {
    var content = '<label>' + data.name + '</label>';
    content += "<select class='form-control' name='" + Encode.forHtml(data.id) +"'>";

    if(data.value) {
        var items = JSON.parse(data.value);
        for(var i = 0; i < items.length; i++) {
            content += "<option value='" + Encode.forHtml(items[i]._id) + "'>" 
            + Encode.forHtml(items[i].text) + "</option>";
        }
    } 
    content += "</select>";
...
function genSimpleTable(data,  disabled)   {
    ...
    content += data.name + ": ";
    content += "</td></tr><br>";
    content += "<table border=\"1\" cellpadding=\"50\" style=\"font-size:14;\"><tr>";
    content += "<th style='padding:5px'>User</th>";
    content += "<th style='padding:5px'>Fact id</th>";
    content += "</tr>";

    for (var i = 0; i < list.length; i++) {
        content += "<tr>";
        content += "<td style='padding:5px'>" + list[i].split(":")[0] + "</td>";
        content += "<td style='padding:5px'>" + list[i].split(":")[1] + "</td>";
        content += "</tr>";
    }
