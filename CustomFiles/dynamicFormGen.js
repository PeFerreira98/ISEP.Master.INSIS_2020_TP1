...
data
{
    ...
    [
        {
            "_id": "0",
            "text": "nameAreaConhecimento0"
        },
        {
            "_id": "1",
            "text": "nameAreaConhecimento1"
        }
    ]
}
*/
...
function genSimpleSelect(data, disabled) {
    ...
    if(data.value){
        ...
        for(var i = 0; i < items.length; i++) {
            content += "<option value='" + Encode.forHtml(items[i]._id) + "'>" + Encode.forHtml(items[i].text) + "</option>";
        }
    }
...
function genSimpleTable(data, disabled) {
    var content = '<label>' + data.name + '</label>';
    content += "<select class='form-control' name='" + Encode.forHtml(data.id) + "' multiple>";
